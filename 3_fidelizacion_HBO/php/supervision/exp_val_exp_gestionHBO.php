...
<head><meta http-equiv="Content-Type" content="text/html; charset=gb18030">
  ...
  <link rel="stylesheet"
    ...
    <div class="modal-body"> 
      <table id="example9090" class="table table-bordered table-striped">
        <thead>
            <tr>
				<th>Codigo Excel</th>
				<th>Cliente</th>
            ...
            <tbody>
				<?php
				    if($a2 == '-1'){
				       ...
				       $block = mysqli_query($enlace, "SELECT CONCAT(u.1er_nombre,' ', u.1er_apellido) As Vendedor, `codigo_excel`,`nombre`,`estado_venta`,`fecha_atencion`, `observacion` FROM HBOFidelizacion hbo INNER JOIN usuarios u ON u.id=hbo.cod_vendedor WHERE `fecha_atencion` BETWEEN '$fch1' AND '$fch2'"); 
				    }else{
				       $block = mysqli_query($enlace, "SELECT CONCAT(u.1er_nombre,' ', u.1er_apellido) As Vendedor, `codigo_excel`,`nombre`,`estado_venta`,`fecha_atencion`, `observacion` FROM HBOFidelizacion hbo INNER JOIN usuarios u ON u.id=hbo.cod_vendedor WHERE `fecha_atencion` BETWEEN '$fch1' AND '$fch2' AND hbo.cod_vendedor='$a2'");
				    }
					mysqli_data_seek ($block, 0);
					while($rows=mysqli_fetch_array($block)){
				?>
					<tr>
						<td><?php echo $rows['codigo_excel'] ?></td>
...
<script>
  ...
  $(function () {
    $('#example9090').DataTable( {
        ...
        language: {
            "decimal":        "",
            "emptyTable":     "No hay registro de llamadas Sin Audio",
    ...
    function descargar_excel(){
       ...
       self.location = "3_fidelizacion_HBO/php/supervision/exp_dowload_excelHBO.php?a19="+a1+"&a22="+a2+"&a33="+a3;
    }
