      ...
      <table id="example29" class="table table-bordered table-striped">
        <thead>
            ...
            <tbody>
				<?php
				    if($a2 == '-1'){
				       $block = mysqli_query($enlace, "SELECT CONCAT(u.1er_nombre,' ', u.1er_apellido) As Asesor, u.usuario, g.cedula, g.anillamador, g.fecha_call, g.omnicanal, g.estado, g.observacion FROM gestion g INNER JOIN usuarios u ON u.id=g.id_usuario WHERE g.fecha_call BETWEEN '$fch1' AND '$fch2'"); 
				    }else{
				       $block = mysqli_query($enlace, "SELECT CONCAT(u.1er_nombre,' ', u.1er_apellido) As Asesor, u.usuario, g.cedula, g.anillamador, g.fecha_call, g.omnicanal, g.estado, g.observacion FROM gestion g INNER JOIN usuarios u ON u.id=g.id_usuario WHERE g.fecha_call BETWEEN '$fch1' AND '$fch2' AND g.id_usuario='$a2'");
				    }
...
<script>
  ...
  $(function () {
    ...
    function descargar_excel(){
       ...
       self.location = "php/supervision/exp_dowload_excelGE.php?a19="+a1+"&a22="+a2+"&a33="+a3;
    }
