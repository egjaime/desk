...
<head><meta http-equiv="Content-Type" content="text/html; charset=gb18030">
  ...
  <link rel="stylesheet"
    ...
    <div class="modal-body"> 
      <table id="example29" class="table table-bordered table-striped">
        <thead>
            <tr>
				<th>Asesor</th>
				<th>usuario open</th>
            ...
            <tbody>
				<?php
				    if($a2 == '-1'){
				       $block = mysqli_query($enlace, "SELECT CONCAT(u.1er_nombre,' ', u.1er_apellido) As Asesor, gvt.usuario, g.fecha_call, g.cedula, gvt.tipo_danio, gvt.num_reclamo, gvt.armario, gvt.virtual, g.observacion FROM gestion_vt gvt INNER JOIN usuarios u ON u.id=gvt.id_usuario INNER JOIN gestion g ON g.id=gvt.id_gestion WHERE fecha_call BETWEEN '$fch1' AND '$fch2'"); 
				    }else{
				       $block = mysqli_query($enlace, "SELECT CONCAT(u.1er_nombre,' ', u.1er_apellido) As Asesor, gvt.usuario, g.fecha_call, g.cedula, gvt.tipo_danio, gvt.num_reclamo, gvt.armario, gvt.virtual, g.observacion FROM gestion_vt gvt INNER JOIN usuarios u ON u.id=gvt.id_usuario INNER JOIN gestion g ON g.id=gvt.id_gestion WHERE fecha_call BETWEEN '$fch1' AND '$fch2' AND gvt.id_usuario='$a2'");
				    }
				...
				?>
					<tr>
						...
						<?php if ($rows['tipo_danio'] === "1"){?>
						   <td>Alineación de antena</td> 
						<?php }else if ($rows['tipo_danio'] === "2"){ ?>
						   <td>Falla cable RCA/HDMI/Poder</td>
						<?php }else if ($rows['tipo_danio'] === "3"){ ?>
						   <td>Falla cable coaxial</td>
						<?php }else if ( $rows['tipo_danio'] === "4"){ ?>
						   <td>Parametros en cero</td>
						<?php }else if ( $rows['tipo_danio'] === "5"){ ?>
						   <td>Posible deco dañado</td> 
						<?php }else if ( $rows['tipo_danio'] === "6"){ ?>
						   <td>CNRP - Sin Señal</td>
						<?php }else if ( $rows['tipo_danio'] === "7"){ ?>
						   <td>Daño LNB</td>
						<?php } ?>
						<td><?php echo $rows['num_reclamo'] ?></td>
						<td><?php echo $rows['armario'] ?></td>
...
<script>
  ...
  $(function () {
    $('#example29').DataTable( {
        ...
        language: {
            "decimal":        "",
            "emptyTable":     "No hay registro de gestion VT",
    ...
    function descargar_excel(){
       ...
       self.location = "php/supervision/exp_dowload_excelVT.php?a19="+a1+"&a22="+a2+"&a33="+a3;
    }
