      ...
      <table id="example29" class="table table-bordered table-striped">
        <thead>
            ...
            <tbody>
				<?php
				    if($a2 == '-1'){
				       $block = mysqli_query($enlace, "SELECT CONCAT(u.1er_nombre,' ', u.1er_apellido) As Asesor, gvt.user_genesis, gvt.id_genesis, g.anillamador, g.fecha_call, g.hora_inicio, gvt.hora_cierre, gvt.motivo_cierre, g.observacion FROM gestion_sa gvt INNER JOIN usuarios u ON u.id=gvt.id_usuario INNER JOIN gestion g ON g.id=gvt.id_gestion WHERE fecha_call BETWEEN '$fch1' AND '$fch2'"); 
				    }else{
				       $block = mysqli_query($enlace, "SELECT CONCAT(u.1er_nombre,' ', u.1er_apellido) As Asesor, gvt.user_genesis, gvt.id_genesis, g.anillamador, g.fecha_call, g.hora_inicio, gvt.hora_cierre, gvt.motivo_cierre, g.observacion FROM gestion_sa gvt INNER JOIN usuarios u ON u.id=gvt.id_usuario INNER JOIN gestion g ON g.id=gvt.id_gestion WHERE fecha_call BETWEEN '$fch1' AND '$fch2' AND gvt.id_usuario='$a2'");
				    }
				...
				?>
					<tr>
						...
						<td><?php echo $rows['user_genesis'] ?></td>
						<td><?php echo $rows['id_genesis'] ?></td>
						<td><?php echo $rows['anillamador'] ?></td>
                        <td><?php echo $rows['fecha_call'] ?></td>
						<td><?php echo $rows['hora_inicio'] ?></td>
						<td><?php echo $rows['hora_cierre'] ?></td>
						<?php if ($rows['motivo_cierre'] === "1"){?>
						   <td>Desconexión Remota</td> 
						<?php }else if ($rows['motivo_cierre'] === "2"){ ?>
						   <td>Desconexión Local</td>
						<?php }else if ($rows['motivo_cierre'] === "3"){ ?>
						   <td>Otro..</td>
						<?php } ?>
...
<script>
  ...
  $(function () {
    ...
    function descargar_excel(){
       ...
       self.location = "php/supervision/exp_dowload_excelSA.php?a19="+a1+"&a22="+a2+"&a33="+a3;
    }
