...
<head><meta http-equiv="Content-Type" content="text/html; charset=gb18030">
  ...
  <link rel="stylesheet"
    ...
    <div class="modal-body"> 
      <table id="example2933451" class="table table-bordered table-striped">
        <thead>
            <tr>
                <th>Usuario</th>
            ...
            <tbody>
				<?php
				    $block = mysqli_query($enlace, "SELECT * FROM act_sva WHERE id_usuario=$a1 AND fecha_act BETWEEN '$fch1' AND '$fch2'"); 
				    mysqli_data_seek ($block, 0);
					while($rows=mysqli_fetch_array($block)){
				?>
					<tr>
					    <td><?php echo $rows['usuario'] ?></td>
						<td><?php echo $rows['fecha_act'] ?></td>
						<td><?php echo $rows['paquete'] ?></td>
						<td><?php echo $rows['ns_ser'] ?></td>
						<td><?php echo $rows['telf_contacto'] ?></td>
					</tr>
				<?php
					} 
				?>
...
<script>
     ...
     function descargar_excel2(){
       ...
       self.location = "php/asesores/act_sva/exp_dowload_excelSVA.php?a19="+a1+"&a22="+a2+"&a33="+a3;
    }
   
  $(function () {
    $('#example2933451').DataTable( {
        "destroy": true,
        "pageLength": 10,
        "searching": false,
        "lengthChange": false,
        language: {
            "decimal":        "",
            "emptyTable":     "No se encontraron activaciones SVA",
