<?php
    ...
    if($a3 == '-1'){
       $block = mysqli_query($enlace, "SELECT count(*) as total FROM gestion_colega g INNER JOIN usuarios u ON u.id=g.id_user WHERE g.fecha BETWEEN '$a1' AND '$a2'"); 
    }else{
       $block = mysqli_query($enlace, "SELECT count(*) as total FROM gestion_colega g INNER JOIN usuarios u ON u.id=g.id_user WHERE g.fecha BETWEEN '$a1' AND '$a2' AND g.id_user='$a3'");
    }
...
<?php
    if($a3 == '-1'){
       $block = mysqli_query($enlace, "SELECT CONCAT(u.1er_nombre,' ', u.1er_apellido) As Asesor, u.usuario, g.ns_splitter, g.fecha,  g.omnicanal, g.estado, g.observacion FROM gestion_colega g INNER JOIN usuarios u ON u.id=g.id_user WHERE g.fecha BETWEEN '$a1' AND '$a2'"); 
    }else{
       $block = mysqli_query($enlace, "SELECT CONCAT(u.1er_nombre,' ', u.1er_apellido) As Asesor, u.usuario, g.ns_splitter, g.fecha,  g.omnicanal, g.estado, g.observacion FROM gestion_colega g INNER JOIN usuarios u ON u.id=g.id_user WHERE g.fecha BETWEEN '$a1' AND '$a2' AND g.id_user='$a3'");
    }
	mysqli_data_seek ($block, 0);
	while($res=mysqli_fetch_array($block)){
  ...
  <tr style="text-align:left;">
    ...
    <td style="width:110px; text-align:center;"><?php echo $omnicanal; ?></td>
		...
		<?php }else if ($estado === "2"){ ?>
		   <td style="width:110px; text-align:center;">Escalado</td>
		<?php }else if ($estado === "3"){ ?>
		   <td style="width:110px; text-align:center;">Pendiente Omnicanal</td>
