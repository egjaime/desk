<?php
	...
	$block = mysqli_query($enlace, "SELECT COUNT(*) as total, SUM(tmo) as TMO FROM gestion_colega WHERE id_user='{$_SESSION['id']}' AND fecha BETWEEN '$fch1' AND '$fch2'");
	mysqli_data_seek ($block, 0);
	$row = mysqli_fetch_array($block);
	
	$block2 = mysqli_query($enlace, "SELECT COUNT(*) as resuelto FROM gestion_colega WHERE id_user='{$_SESSION['id']}' AND fecha BETWEEN '$fch1' AND '$fch2' AND estado=1");
	mysqli_data_seek ($block2, 0);
	$row2 = mysqli_fetch_array($block2);
	
	$block3 = mysqli_query($enlace, "SELECT COUNT(*) as escalado FROM gestion_colega WHERE id_user='{$_SESSION['id']}' AND fecha BETWEEN '$fch1' AND '$fch2' AND estado=2");
	mysqli_data_seek ($block3, 0);
	$row3 = mysqli_fetch_array($block3);
	
	$block5 = mysqli_query($enlace, "SELECT COUNT(*) as sinaudio FROM gestion_colega WHERE id_user='{$_SESSION['id']}' AND fecha BETWEEN '$fch1' AND '$fch2' AND estado=5");
	mysqli_data_seek ($block5, 0);
	$row5 = mysqli_fetch_array($block5);
	
	$blockF = mysqli_query($enlace, "SELECT COUNT(*) as total FROM gestion_colega WHERE id_user='{$_SESSION['id']}' AND fecha BETWEEN '$fch1' AND '$fch2'");
