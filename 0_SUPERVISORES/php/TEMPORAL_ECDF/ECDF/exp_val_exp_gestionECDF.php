...
<head><meta http-equiv="Content-Type" content="text/html; charset=gb18030">
  ...
  <link rel="stylesheet"
    ...
    <div class="modal-body"> 
      <table id="example293345193_2" class="table table-bordered table-striped">
        <thead>
            <tr>
                <th>Asesor</th>
                <th>Cedula - NS</th>
            ...
            <tbody>
				<?php
				    if($a1==="-1"){
				      $block = mysqli_query($enlace, "SELECT *,CONCAT(u.1er_nombre,' ', u.1er_apellido) As nombre_as FROM HBOFidelizacion e INNER JOIN usuarios u ON u.id=e.cod_vendedor WHERE e.fecha_atencion BETWEEN '$fch1' AND '$fch2'");
				    }else{
				        $block = mysqli_query($enlace, "SELECT *,CONCAT(u.1er_nombre,' ', u.1er_apellido) As nombre_as FROM HBOFidelizacion e INNER JOIN usuarios u ON u.id=e.cod_vendedor WHERE cod_vendedor='$a1' AND e.fecha_atencion BETWEEN '$fch1' AND '$fch2'"); 
				    }
...
<script>
     ...
     function descargar_excel2(){
       ...
       self.location = "0_SUPERVISORES/php/TEMPORAL_ECDF/ECDF/exp_dowload_excelECDF.php?a19="+a1+"&a22="+a2+"&a33="+a3;
    }
   
  $(function () {
    $('#example293345193_2').DataTable( {
        "destroy": true,
        "pageLength": 10,
        "searching": false,
        "lengthChange": false,
        ...
        language: {
            "decimal":        "",
            "emptyTable":     "No se encontraron activaciones de ECDF",
