...
<head><meta http-equiv="Content-Type" content="text/html; charset=gb18030">
  ...
  <link rel="stylesheet"
    ...
    <div class="modal-body"> 
      <table id="example78797019" class="table table-bordered table-striped">
        <thead>
            <tr>
            ...
            <tbody>
				<?php
				    ...
				    if($a1==="-1"){
				      $block = mysqli_query($enlace, "SELECT *,CONCAT(u.1er_nombre,' ', u.1er_apellido) As Nombre FROM retenciones e INNER JOIN usuarios u ON u.id=e.id_user WHERE e.fecha_hora BETWEEN '$fch1' AND '$fch2'");
				    }else{
				      $block = mysqli_query($enlace, "SELECT *,CONCAT(u.1er_nombre,' ', u.1er_apellido) As Nombre FROM retenciones e INNER JOIN usuarios u ON u.id=e.id_user WHERE e.id_user='$a1' AND e.fecha_hora BETWEEN '$fch1' AND '$fch2'"); 
				    }
				...
				?>
					<tr>
					    ...
					    <?php if($rows['estado'] == "1"){?>
					        <td><?php echo 'Retenido' ?></td>
					    <?php }else if($rows['estado'] == "2"){?>
					        <td><?php echo 'No Retenido' ?></td>
...
<script>
     ...
     function descargar_excel2(){
       ...
       self.location = "0_SUPERVISORES/php/RETENCIONES/GESTION_RETENCIONES/exp_dowload_excelSVA.php?a19="+a1+"&a22="+a2+"&a33="+a3;
    }
   
  $(function () {
    $('#example78797019').DataTable( {
        "destroy": true,
        "pageLength": 10,
        "searching": false,
        "lengthChange": false,
        ...
        language: {
            "decimal":        "",
            "emptyTable":     "No se encontraron Retenciones",
