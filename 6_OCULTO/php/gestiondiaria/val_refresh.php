<?php
	...
	$block = mysqli_query($enlace, "SELECT COUNT(*) as total FROM gestion_oculto WHERE id_user='{$_SESSION['id']}' AND fecha_registro BETWEEN '$fch1' AND '$fch2'");
	mysqli_data_seek ($block, 0);
	$row = mysqli_fetch_array($block);
	
	$block2 = mysqli_query($enlace, "SELECT COUNT(*) as exitoso FROM gestion_oculto WHERE id_user='{$_SESSION['id']}' AND fecha_registro BETWEEN '$fch1' AND '$fch2' AND gestion_exitosa='SI'");
	mysqli_data_seek ($block2, 0);
	$row2 = mysqli_fetch_array($block2);
	
	$block3 = mysqli_query($enlace, "SELECT COUNT(*) as exitoso FROM gestion_oculto WHERE id_user='{$_SESSION['id']}' AND fecha_registro BETWEEN '$fch1' AND '$fch2' AND gestion_exitosa='NO'");
