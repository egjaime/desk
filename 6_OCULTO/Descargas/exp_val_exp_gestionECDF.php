...
<head><meta http-equiv="Content-Type" content="text/html; charset=gb18030">
  ...
  <link rel="stylesheet"
    ...
    <div class="modal-body"> 
      <table id="example2933451939911" class="table table-bordered table-striped">
        <thead>
            <tr>
                <th>Asesor</th>
            ...
            <tbody>
				<?php
				    if($a1==="-1"){
				      $block = mysqli_query($enlace, "SELECT *,CONCAT(u.1er_nombre,' ', u.1er_apellido) As nombre_as FROM gestion_oculto e INNER JOIN usuarios u ON u.id=e.id_user WHERE e.fecha_registro BETWEEN '$fch1' AND '$fch2'");
				    }else{
				        $block = mysqli_query($enlace, "SELECT *,CONCAT(u.1er_nombre,' ', u.1er_apellido) As nombre_as FROM gestion_oculto e INNER JOIN usuarios u ON u.id=e.id_user WHERE id_user='$a1' AND e.fecha_registro BETWEEN '$fch1' AND '$fch2'"); 
				    }
				...
				?>
					<tr>
					    ...
					    <td><?php echo $rows['hora_registro'] ?></td>
					    <td><?php echo $rows['nombre_cliente'] ?></td>
						<td><?php echo $rows['cedula_cliente'] ?></td>
						<td><?php echo $rows['ns'] ?></td>
						<td><?php echo $rows['gestion_exitosa'] ?></td>
						<td><?php echo $rows['tipificacion'] ?></td>
...
<script>
     ...
     function descargar_excel2(){
       ...
       self.location = "6_OCULTO/Descargas/exp_dowload_excelECDF.php?a19="+a1+"&a22="+a2+"&a33="+a3;
    }
   
  $(function () {
    $('#example2933451939911').DataTable( {
        "destroy": true,
        "pageLength": 20,
        "searching": false,
        "lengthChange": false,
        ...
        language: {
            "decimal":        "",
            "emptyTable":     "No se encontraron Gestiones DTH Oculto",
