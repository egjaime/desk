...
<head><meta http-equiv="Content-Type" content="text/html; charset=gb18030">
  ...
  <link rel="stylesheet"
    ...
    <div class="modal-body"> 
      <table id="example293345193_22" class="table table-bordered table-striped">
        <thead>
            <tr>
                <th>Asesor</th>
                <th>Fecha</th>
            ...
            <tbody>
				<?php
				    if($a1==="-1"){
				      $block = mysqli_query($enlace, "SELECT *,CONCAT(u.1er_nombre,' ', u.1er_apellido) As nombre_as FROM INT_ic_open e INNER JOIN usuarios u ON u.id=e.id_user WHERE e.fecha BETWEEN '$fch1' AND '$fch2'");
				    }else{
				        $block = mysqli_query($enlace, "SELECT *,CONCAT(u.1er_nombre,' ', u.1er_apellido) As nombre_as FROM INT_ic_open e INNER JOIN usuarios u ON u.id=e.id_user WHERE id_user='$a1' AND e.fecha BETWEEN '$fch1' AND '$fch2'"); 
				    }
				...
				?>
					<tr>
						...
						<td><?php echo $rows['n_servicio'] ?></td>
						<td><?php echo $rows['n_queja'] ?></td>
						<td><?php echo $rows['estado_ic'] ?></td>
						<td><?php echo $rows['estado_open'] ?></td>
...
<script>
     ...
     function descargar_excel2(){
       ...
       self.location = "8_ESPECIFICAS_INT/supervision/descargas/exp_dowload_excel2.php?a19="+a1+"&a22="+a2+"&a33="+a3;
    }
   
  $(function () {
    $('#example293345193_22').DataTable( {
        "destroy": true,
        "pageLength": 10,
        "searching": false,
        "lengthChange": false,
